<script lang="ts">
	...
	import type { SingleMessageModel } from '../../../data/singleMessage';
	...
	{#if message.isUser}
		...
		<div class="flex items-start space-x-4">
			<div class="max-w-xs rounded-lg border border-gray-300 p-2">
				...
				<div class="text-right text-gray-700">{message.messageContent}</div>
			</div>

			<Avatar src="src/assets/user.jpg" />
		</div>
	{:else}
		...
		<div class="flex items-start space-x-4">
			...
			<div class="max-w-xs rounded-lg border border-gray-300 p-2">
				...
				<div class="text-gray-700">{message.messageContent}</div>
			</div>
		</div>
	{/if}
</div>
